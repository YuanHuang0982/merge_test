...
from Mpruning import MambaAbsoluteThresholdPruner
...
class SS2Dv2:
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
        x = self.act(x)
        y = self.forward_core(x, selective_scan_backend=self.backend)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        layer_idx: int = 0, 
        total_layers: int = 0,
        final_token_threshold: float = 0.15,
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            # pruning 기준선을 가진 pruner 추가
            self.pruner = MambaAbsoluteThresholdPruner(
                layer_idx=layer_idx,
                total_layers=total_layers,
                final_token_threshold=final_token_threshold,
                method="abc"  # 원하시는 방식으로 선택
            )
    ...
    def _ssm_only_kept(self, y_kept: torch.Tensor) -> torch.Tensor:
        # (B, K_max, C) → (B, C, 1, K_max) → self.op → 다시 (B, K_max, C)
        out = self.op(y_kept.permute(0, 2, 1).unsqueeze(2))
    ...
    def _forward(self, input: torch.Tensor):
        ...
        x_norm = self.norm(x)
        if self.ssm_branch:
            # 1) 기존 SSM 연산: post_norm 여부에 따라 순서 결정 → y: (B, N, C)
            if self.post_norm:
                y = self.op(x)
                y = self.norm(y)
                # pruning 에는 원본 x 를
            ...
            # 2) Flatten → (B, N, C)
            B, H, W, C = y.shape
            y_seq = y.reshape(B, H*W, C)

            prev_x_seq = prev_x.reshape(B, H*W, C)
            # 3) Pruner 적용: 중요도 낮은 토큰은 0으로, 위치 그대로 유지
            #    delta 방식이라면 prev_x_seq를 넘겨주고, abc 방식이라면 B_mat/C_mat도 넘겨주세요.
            #    예: y_pruned, prune_info = self.pruner(y, prev_x=prev_x_seq)
            y_pruned_seq, prune_info = self.pruner(y_seq,prev_x=prev_x_seq)

            logger.info(f"[Layer {self.layer_idx}] prune_info keys: {list(prune_info.keys())}")
            num_pruned = prune_info.get('num_pruned',
                            prune_info.get('pruned_count',
                            len(prune_info.get('pruned_indices', []))))
            num_kept   = prune_info.get('num_kept',
                            prune_info.get('kept_count',
                            len(prune_info.get('kept_indices', []))))

            logger.info(
                f"[Layer {self.layer_idx}] pruned {num_pruned} / {num_pruned + num_kept} tokens"
            )



            # 2) 실제 0 마스크 확인
            zero_mask = (y_pruned_seq.abs().sum(dim=-1) == 0)   # (B, N)
            num_pruned = zero_mask.sum().item()
            total_tokens = y_pruned_seq.size(0) * y_pruned_seq.size(1)
            logger.info(f"[Layer {self.layer_idx}] 실제 pruned "
                        f"{num_pruned}/{total_tokens} "
                        f"({100*num_pruned/total_tokens:.1f}%)")
            ...
            # 4) Reshape → (B, H, W, C)
            y_pruned = y_pruned_seq.reshape(B, H, W, C)


            # 5) residual + drop_path
            x = x + self.drop_path(y_pruned)
            '''

        x = input
        x_norm = self.norm(x)

        # 1) Prepare raw sequence for pruning
        x_for_seq = x if self.post_norm else x_norm
        if self.channel_first:
            x_for_seq = x_for_seq.permute(0, 2, 3, 1).contiguous()  # (B, H, W, C)
        B, H, W, C = x_for_seq.shape
        x_seq = x_for_seq.reshape(B, H * W, C)  # (B, N, C)

        # 2) Pruning → get remaining token mask
        # y_seq, prune_info = self.pruner(x_seq, prev_x=x_seq) Delta


        if self.channel_first:
            tmp_full = x_for_seq.permute(0, 3, 1, 2)   # (B, C, H, W)
        else:
            tmp_full = x_for_seq                      # (B, H, W, C)
        _ = self.op(tmp_full)  # debug 모드 켜져 있으니 self.op.__data__에 Bs, Cs가 저장됨
        B_mat = self.op.__data__['Bs']  # (B, K, d_state, L)
        C_mat = self.op.__data__['Cs']  # (B, K, d_state, L)
        y_seq, prune_info = self.pruner(x_seq,B_mat=B_mat,C_mat=C_mat)
        ...
        mask = prune_info['mask'].bool()      # (B, N), bool tensor

        # 3) Pad kept tokens → gather 버전
        B, N, C = x_seq.shape
        lengths = mask.sum(dim=1)              # (B,)
        K_max = int(lengths.max().item())

        idx = torch.zeros(B, K_max, dtype=torch.long, device=x_seq.device)
        for b in range(B):
            kept = mask[b].nonzero(as_tuple=False).squeeze(-1)
            idx[b, :kept.size(0)] = kept

        y_kept_pad = x_seq.gather(
            dim=1,
        ...
        # 4) SSM 연산 (기존과 동일)
        if self.channel_first:
            tmp = y_kept_pad.permute(0, 2, 1).unsqueeze(2)
        else:
            tmp = y_kept_pad.unsqueeze(1)
        y_kept_out = self.op(tmp)
        if self.channel_first:
            y_kept_out = y_kept_out.squeeze(2).permute(0, 2, 1)
        else:
            y_kept_out = y_kept_out.squeeze(1)
        y_kept_out = y_kept_out.to(x_seq.dtype)

        # 5) Scatter back → (B, N, C)
        y_seq_out = x_seq.new_zeros(B, N, C)
        y_seq_out.scatter_(
            dim=1,
            index=idx.unsqueeze(-1).expand(-1, -1, C),
            src=y_kept_out
        ...
        # 6) Restore spatial & residual
        y = y_seq_out.view(B, H, W, C)
        if self.channel_first:
        ...
        **kwargs,
        ...
        for d in range(depth):
            blocks.append(VSSBlock(
                ...
                layer_idx           = d + 1,       # 1-based index
                total_layers        = depth,       # 전체 블록 수
                final_token_threshold = 0.3,      # 원하는 pruning 강도 (0~1)
