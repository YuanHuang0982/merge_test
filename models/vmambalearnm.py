...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.block_index = layer_idx


        if self.token_merge:
            self.merge_module = CrossLayerMerge(
                layer_idx = self.block_index + 1,       # 1-based index
                total_layers = self.total_blocks,
    ...
    def _forward(self, input: torch.Tensor, prev_features: Optional[list] = None):
        ...
        if self.token_merge:
            ...
            if condition:
                if x.ndim == 4:
                    ...
                    # 🔴 토큰 개수가 최소 2 이상일 때만 merge
                    if N_ori >= 2:
                        #print(f"[Merge] Block {self.block_index}: 토큰 {N_ori}개 → threshold {self.final_threshold}")
                        # (너무 작아서 merge 의미 없으면 건너뛰고 다음으로)
                        ...
                        # 🔴 learnable threshold based merge
                        # 1) metric 계산
                        metric = x_flat / x_flat.norm(dim=-1, keepdim=True)
                        # 2) merge_fn 얻기
                        merge_fn, _ = self.merge_module(
                            metric,
                            class_token=False,
                            distill_token=False
                        )
                        # 3) 실제 병합
                        x_merged = merge_fn(x_flat, mode="mean")
        ...
        if self.mlp_branch:
            # ─── MLP 분기 전에 채널‐첫/채널‐라스트 layout 맞추기 ───
            if self.channel_first:
                # x가 (B,C,H,W)인지 확인
                # 만약 merge 로직에서 실수로 (B,H,W,C) 로 나온 경우가 있다면 permute
                if x.shape[1] != x.size(-1): 
                    # x가 (B,H,W,C) 이면
                    x = x.permute(0, 3, 1, 2).contiguous()
            else:
                # x가 (B,H,W,C)인지 확인
                if x.shape[-1] != x.size(1):
                    x = x.permute(0, 2, 3, 1).contiguous()
            # ─── 이제 안전하게 LayerNorm2d or LayerNorm 호출 ───
            res = self.mlp(self.norm2(x)) if not self.post_norm else self.norm2(self.mlp(x))
            x = x + self.drop_path(res)

            # ─── MLP 이후에 원래 layout으로 복원 ───
            if self.channel_first:
                # 이미 (B,C,H,W) 이므로 아무 것도 안 해도 됩니다.
                pass
            else:
                # MLP 결과가 (B,C,H,W) 로 나왔다면
                if res.ndim == 4 and res.shape[1] == x.shape[-1]:  
                    x = x.permute(0, 2, 3, 1).contiguous()
...
class VSSM(nn.Module):
    def __init__(
        ...
        token_merge=True,
        #fusion_token=32,
        final_threshold=0.5,      # ← 기본값 설정 (0~1 사이)
