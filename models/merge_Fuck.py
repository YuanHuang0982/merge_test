...
from token_merge import bipartite_soft_matching, merge_wavg
...
def visualize_cosine_similarity(
    ...
    """
    - hidden_states: (B, N, D)
    - layer_idx: 해당 블록 인덱스 (파일명에 사용)
    - max_tokens: 최대 토큰 샘플 수
    - out_dir: 결과 저장 디렉터리
    """
    B, N, D = hidden_states.shape
    # 1) 첫 배치만
    hs = hidden_states[0]        # (N, D)
    # 2) 토큰 샘플링
    if N > max_tokens:
        idx = torch.randperm(N)[:max_tokens]
        hs = hs[idx]
    # 3) NumPy 변환
    x = hs.detach().cpu().numpy()  # (k, D)
    # 4) 코사인 유사도 계산
    sim = cosine_similarity(x)     # (k, k)
    ...
    plt.title(f"Layer {layer_idx} Cosine Similarity (k={x.shape[0]})")
    filepath = os.path.join(out_dir, f"cosine_layer{layer_idx}.png")
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        # =============================
        token_merge=True,  # 추가
        fusion_token=11500,     # 추가
        fusion_layer=5,     # 추가
        block_index=0,        # 추가
        fusion_strategy='all',  # 추가
        ...
        **kwargs,
        ...
        self.merge_strategy = fusion_strategy
        self.merge_layer = fusion_layer
    ...
    def _forward(self, input: torch.Tensor, prev_features: Optional[list] = None):
        x = input
        #print(f"[DEBUG] Block {self.block_index}, token_merge={self.token_merge}, fusion_token={self.fusion_token}, strategy={self.merge_strategy}, merge_layer={self.merge_layer}")
        ...
        if prev_features:
            ...
            for pf in prev_features:
                if pf.shape == x.shape:
                    compatible_features.append(pf)
                else:
                    #print(f"[WARNING] Skip connection dropped due to shape mismatch: {pf.shape} vs {x.shape}")
                    None
            if len(compatible_features) > 0:
                fused = torch.stack(compatible_features + [x], dim=0)
        ...
        if self.token_merge:
            condition = (
                (self.merge_strategy == 'upper' and self.block_index < self.merge_layer) or
                (self.merge_strategy == 'lower' and self.block_index > self.merge_layer) or
                (self.merge_strategy == 'alternate' and self.block_index % 2 == 1) or
                (self.merge_strategy == 'all')
            )
            
            if condition and self.fusion_token > 0:
                if x.ndim == 4:
                    # 0) channel-first 이면 channel-last 로 변환
                    if self.channel_first:
                        x = x.permute(0, 2, 3, 1).contiguous()
     
                    # 1) 이제 (B,H,W,C)로 unpack
                    B, H_ori, W_ori, C = x.shape
                    #print(f"[DBG] merge 전 H={H_ori}, W={W_ori}, C={C}")
                        
                    N_ori = H_ori * W_ori

                    x_flat = x.reshape(B, N_ori, C)
                    # 2) Token Merge → (B, R, C)
                    keep_tokens = self.fusion_token
                    remove_tokens = max(0, N_ori - keep_tokens)
                    merge, _ = bipartite_soft_matching(x_flat, remove_tokens, class_token=False)
                    x_merged = merge(x_flat)  # (B, R, C)
                    self.x_merged = x_merged  # ← Token Merge 이후 시각화를 위한 저장
                    
                    R = x_merged.shape[1]
                    # 3) 채널 수가 그대로인지 점검
                    B2, R, C2 = x_merged.shape
                    assert B2 == B and C2 == C, f"채널 수 변형됨: {C2} != {C}"

                    # 4) R을 정방(square) 형태로 되돌릴 높이/너비 계산
                    new_H = int(math.sqrt(R))
                    new_W = R // new_H
                    if new_H * new_W != R:
                        pad_len = new_H * new_W - R
                        x_merged = F.pad(x_merged, (0, 0, 0, pad_len))
                        R = new_H * new_W
                    x_merged = x_merged.reshape(B, new_H, new_W, C)
                    # 6) channel_first 옵션에 따라 (B,C,H,W) 또는 (B,H,W,C) 포맷 유지
                    if self.channel_first:
                        # VSSM.channel_first=True 이면 내부에서 (B,C,H,W) 써야 하므로
                        x = x_merged.permute(0, 3, 1, 2).contiguous()
                    else:
                        # 기본 포맷인 (B,H,W,C) 그대로
                        x = x_merged
            ...
            #print(f"[DEBUG] After merge: block {self.block_index}, x.shape = {x.shape}")  
        ...
        if self.visualize and x.ndim == 4 and not self.training:
            # (B, H, W, C) → (B, N=H*W, C)
            B, H, W, C = x.shape
            hs = x.permute(0, 2, 3, 1).reshape(B, H*W, C)
            
            # 동적으로 토큰 수 계산
            P = self.img_size // self.patch_size         # ex. 224//4 = 56
            p = P // (2 ** self.stage_index)            # stage 에 따라 56,28,14,7
            max_tokens = p * p  
                    
            # block_index 속성을 레이어 번호로 넘겨주면 파일명이 구분됩니다
            visualize_cosine_similarity(
                hidden_states=hs,
                layer_idx=self.block_index,
                max_tokens=max_tokens,      # 필요에 따라 조절
                out_dir="./visuals"  # 원하는 경로로 변경 가능
            )
...
class VSSM(nn.Module):
    ...
    def _make_layer(
        ...
        merge_strategy="all",
        merge_layer=12,
        stage_index=0,  # 계층 번호 (stage 단위, optional)
        ...
        **kwargs,
        ...
        for d in range(depth):
            block_index = block_index_offset + d  # block 번호 (해당 stage 내에서)
    ...
    def forward_with_skip(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            prev_features = []  # <- block-level skip 연결 초기화
            for block in layer.blocks:
                x = block(x, prev_features=prev_features)
    ...
    def forward(self, x: torch.Tensor):
        if getattr(self, 'use_skip', True):
            return self.forward_with_skip(x)
        else:
            return self.forward_no_skip(x)  # 안전하게 정의된 fallback

    def set_skip_mode(self, enabled: bool = True):
        self.use_skip = enabled
    
    def forward_no_skip(self, x: torch.Tensor):
        x = self.patch_embed(x)
        if self.pos_embed is not None:
            pos_embed = self.pos_embed.permute(0, 2, 3, 1) if not self.channel_first else self.pos_embed
            x = x + pos_embed
