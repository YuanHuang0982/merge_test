...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor, prev_features: Optional[list] = None):
        ...
        if prev_features:
            # DenseMamba-style skip connection with gating
            fused = torch.stack(prev_features + [x], dim=0)  # [N, B, H, W, C] or [N, B, C, H, W]
            gate = self.skip_gate(x)  # B, H, W, C or B, C, H, W
            if not self.op.channel_first:
                gate = gate.unsqueeze(0)  # [1, B, H, W, C]
            else:
                gate = gate.unsqueeze(0)  # [1, B, C, H, W]
