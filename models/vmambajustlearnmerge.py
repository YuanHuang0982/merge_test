...
from learnMerge import LearnableThresholdSoftMatching
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if self.token_merge:
            self.merge_module = LearnableThresholdSoftMatching(layer_idx=layer_idx,
            ...
            if condition:
                if x.ndim == 4:
                    ...
                    if N >= 2:
                        ...
                        merge_fn, _ = self.merge_module(metric, class_token=False, distill_token=False)
                        ...
                        if new_H*new_W != R:
                            pad = new_H*new_W - R
                            x_merged = F.pad(x_merged, (0,0,0,pad))
                        x = x_merged.view(B, new_H, new_W, C)

                    # back to channel-first if needed
                    if self.channel_first:
                        x = x.permute(0,3,1,2).contiguous()
